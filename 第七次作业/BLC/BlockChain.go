...
func ZjDBExists(dbName string) bool {
...
/**
	创建带有创世区块的区块链
 */
func ZjCreateBlockchainWithGenesisBlock(address string,nodeID string) *Blockchain {
...
// 返回Blockchain对象
func ZjBlockchainObject(nodeID string) *Blockchain {

	dbName := fmt.Sprintf(dbName,nodeID)

	// 判断数据库是否存在
	if ZjDBExists(dbName) == false {
		fmt.Println("数据库不存在....")
		os.Exit(1)
	}
...
// 挖掘新的区块
func (blockchain *Blockchain) ZjMineNewBlock(from []string, to []string, amount []string, nodeID string) {
	//1.建立一笔交易
	...
	for index, address := range from {
		...
		tx := ZjNewSimpleTransaction(address, to[index], int64(value), utxoSet,nodeID)
		txs = append(txs, tx)
	}
...
func (bc *Blockchain) GetBestHeight() int64 {
	...
	block := bc.ZjIterator().ZjNext()

	return block.ZjHeight
}

func (bc *Blockchain) GetBlockHashes() [][]byte {

	blockIterator := bc.ZjIterator()
	...
	for {
		...
		blockHashs = append(blockHashs,block.ZjHash)
...
func (bc *Blockchain) GetBlock(blockHash []byte) ([]byte ,error) {
	...
	err := bc.ZjDB.View(func(tx *bolt.Tx) error {
...
func (bc *Blockchain) AddBlock(block *Block)  {
	...
	err := bc.ZjDB.Update(func(tx *bolt.Tx) error {

		b := tx.Bucket([]byte(blockTableName))

		if b != nil {

			blockExist := b.Get(block.ZjHash)
			...
			err := b.Put(block.ZjHash,block.ZjSerialize())
			...
			blockInDB := ZjDeserializeBlock(blockBytes)

			if blockInDB.ZjHeight < block.ZjHeight {

				b.Put([]byte("l"),block.ZjHash)
				bc.ZjTip = block.ZjHash
			}
		}
